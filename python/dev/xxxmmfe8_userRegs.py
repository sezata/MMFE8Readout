...
class userRegs:
    ...
    def write_button_Reg2_callback(self, widget, textBox):
        ...
        myValue = string.split(VALUE,'x')
        if (len(myValue) == 1):
            MESSAGE = "W 0x44A10108 0x" + myValue[0] + '\0' + '\n'
        elif (len(myValue) == 2):
            MESSAGE = "W 0x44A10108 0x" + myValue[1] + '\0' + '\n'
        else:
            print "ERROR:  Improper value"
            textBox.set_text("Error!")
            return 0
    ...
    def write_button_Reg3_callback(self, widget, textBox):
        ...
        myValue = string.split(VALUE,'x')
        if (len(myValue) == 1):
            MESSAGE = "W 0x44A1010C 0x" + myValue[0] + '\0' + '\n'
        elif (len(myValue) == 2):
            MESSAGE = "W 0x44A1010C 0x" + myValue[1] + '\0' + '\n'
        else:
            print "ERROR:  Improper value"
            textBox.set_text("Error!")
            return 0
    ...
    def write_button_Reg4_callback(self, widget, textBox):
        ...
        myValue = string.split(VALUE,'x')
        if (len(myValue) == 1):
            MESSAGE = "W 0x44A10110 0x" + myValue[0] + '\0' + '\n'
        elif (len(myValue) == 2):
            MESSAGE = "W 0x44A10110 0x" + myValue[1] + '\0' + '\n'
        else:
            print "ERROR:  Improper value"
            textBox.set_text("Error!")
            return 0
    ...
    def write_button_Reg5_callback(self, widget, textBox):
        ...
        myValue = string.split(VALUE,'x')
        if (len(myValue) == 1):
            MESSAGE = "W 0x44A10114 0x" + myValue[0] + '\0' + '\n'
        elif (len(myValue) == 2):
            MESSAGE = "W 0x44A10114 0x" + myValue[1] + '\0' + '\n'
        else:
            print "ERROR:  Improper value"
            textBox.set_text("Error!")
            return 0
...
#        try:
        ...
        if entry > 0xffffffff:
            ...
            Message = "w 0x44A10104 " + str(entry) + '\0' + '\n'
            print "Wrote",hex(entry),"to User Defined Reg 1"
            data = self.udp.udp_client(MESSAGE,self.UDP_IP,self.UDP_PORT)
            myData = string.split(data,'\n')
            textBox.set_text(myData[1])
            ...
            myMsg = "a ValueError exception occurred\nUser Defined Reg 1 Value not hexadecimal."
            self.on_erro(widget, myMsg)
        except:
            print "Unexpected Error Writing to User Defined Reg 1:\n", sys.exc_info()[0]
            self.on_erro(widget, "Unexpected Error:\nDetails printed to shell.")
...
#        try:
        ...
        if entry > 0xffffffff:
            ...
            print "Unexpected Error Writing to User Defined Reg 2:\n", sys.exc_info()[0]
            self.on_erro(widget, "Unexpected Error:\nDetails printed to shell.")
...
#        try:
        ...
        if entry > 0xffffffff:
            ...
            myMsg = "a ValueError exception occurred\nUser Defined Reg 5 Value not hexadecimal."
            self.on_erro(widget, myMsg)
        except:
            print "Unexpected Error Writing to User Defined Reg 5:\n", sys.exc_info()[0]
            self.on_erro(widget, "Unexpected Error:\nDetails printed to shell.")
